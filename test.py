...
def saveLog():
    ...
    with open(+'log.txt', 'a') as f:
        now = time
        num = "저장횟수 : " + str(count) + "\n"
        num_time = "저장시간 : " + now.strftime('%Y-%m-%d %H:%M:%S')+"\n"
        f.write(num)
        f.write(num_time)
    f.close()
    ...
    while True:
        if status:
            if zed.grab(runtime) == sl.ERROR_CODE.SUCCESS:
                ...
                # Save image
                que.put({'left': [os.path.join(path, f'left_0.jpg'), left.get_data()]})
                que.put({'right': [os.path.join(path, f'right_0.jpg'), right.get_data()]})
                que.put({'depth': [os.path.join(path, f'depth_0.jpg'), depth.get_data()]})
                que.put({'dis': [os.path.join(path, f'distance_0.npy'),
                                 DepthNormalizing(distance_undefined(dis.get_data()))]})
                que.put({'slope': [os.path.join(path, f'slope_0.npy'), quaternion]})
                count += 1
                saveLog()
        else:
            count = 0
...
def main():
    ...
    # image resizing
    image_size.width = image_size.width / 1.5
    image_size.height = image_size.height / 1.5
